...
class Probes:
    ...
    def t1_t7_u1_probes(self):
        ...
        T1 = IP(dst=self.host) / TCP(sport=src_port + 1, dport=self.oport, flags="S", options=[
            ("WScale", 10), ("NOP", None), ("MSS", 1460), ("Timestamp", (4294967295, 0)), ("SAckOK", "")], window=1)

        T2 = IP(dst=self.host, flags="DF") / TCP(seq=0, sport=src_port + 2,
                                                 dport=self.oport, options=tcpopt, window=128)

        T3 = IP(dst=self.host) / TCP(seq=0, sport=src_port + 3,
                                     dport=self.oport, options=tcpopt, flags="SFUP", window=256)

        T4 = IP(dst=self.host, flags="DF") / TCP(seq=0, sport=src_port + 4,
                                                 dport=self.oport, options=tcpopt, flags="A", window=1024)

        T5 = IP(dst=self.host) / TCP(seq=0, sport=src_port + 5,
                                     dport=self.cport, options=tcpopt, flags="S", window=31337)

        T6 = IP(dst=self.host, flags="DF") / TCP(seq=0, sport=src_port + 6,
                                                 dport=self.cport, options=tcpopt, flags="A", window=32768)

        T7 = IP(dst=self.host) / TCP(seq=0, sport=src_port + 7,
    ...
    def seq_probes(self):
        ...
        packet1 = IP(dst=self.host) / TCP(sport=src_port + 1, dport=self.oport, flags="S", options=[
            ("WScale", 10), ("NOP", None), ("MSS", 1460), ("Timestamp", (4294967295, 0)), ("SAckOK", "")], window=1)

        packet2 = IP(dst=self.host) / TCP(sport=src_port + 2,  dport=self.oport, flags="S", options=[
            ("MSS", 1400), ("WScale", 0), ("SAckOK", ""), ("Timestamp", (4294967295, 0)), ("EOL", None)], window=63)

        packet3 = IP(dst=self.host) / TCP(sport=src_port + 3, dport=self.oport, flags="S", options=[
            ("Timestamp", (4294967295, 0)), ("NOP", None), ("NOP", None), ("WScale", 5), ("NOP", None), ("MSS", 640)], window=4)

        packet4 = IP(dst=self.host) / TCP(sport=src_port + 4, dport=self.oport, flags="S", options=[
            ("SAckOK", ""), ("Timestamp", (4294967295, 0)), ("WScale", 10), ("EOL", None)], window=4)

        packet5 = IP(dst=self.host) / TCP(sport=src_port + 5, dport=self.oport, flags="S", options=[
            ("WScale", 10), ("MSS", 536), ("SAckOK", ""), ("Timestamp", (4294967295, 0)), ("EOL", None)], window=16)

        packet6 = IP(dst=self.host) / TCP(sport=src_port + 6, dport=self.oport, flags="S", options=[
