...
def arg_parser():
    parser = argparse.ArgumentParser(
        ...
        epilog='To start, run sudo python os_fingerprint.py [host]')
    parser.add_argument("host", help="The IP or hostname/domain of the host")
    parser.add_argument(
        "-f", "--fast", help="Get a faster fingerprint by scanning less ports", action='store_true')
    parser.add_argument(
        "-t", "--timeout", help="Define the timeout for receiving an answer for a packet.\nThe default is 2 seconds.", type=int, dest="timeout")
    parser.add_argument(
